...
class VGSceneLoader(object):
    def __init__(self, vocab, imgs_path, h5_path, masks_json, mode="train", image_size=(256, 256),
                 mask_size = 16, max_objects=10, include_relationships=True, use_orphaned_objects=True):
        """
        # Variables:
        ...
        # Load annotations
        with open(masks_json, 'r') as mask_f:
            print("masks: {}".format(masks_json))
            self.mask_dict = json.load(mask_f)
    ...
    def __getitem__(self, index):
        ...
        for i, obj_idx in enumerate(obj_idxs):
            ...
            obj_id = self.data['object_ids'][index, obj_idx].item()
            if str(obj_id) in self.mask_dict:
                # fix mask
                box_x, box_y, box_w, box_h = x.item(), y.item(), w.item(), h.item()
                rle = self.mask_dict[str(obj_id)]['segmentation']
                mask_h, mask_w = rle['size'][:]
                mask_x0, mask_y0 = int(x0 * mask_w), int(y0 * mask_h)
                mask_x1, mask_y1 = int(x1 * mask_w), int(y1 * mask_h)
                rle_info = mask_utils.frPyObjects(rle, mask_h, mask_w)
                mask = mask_utils.decode(rle_info)
                mx0, mx1 = int(round(mask_x0)), int(round(mask_x1))
                my0, my1 = int(round(mask_y0)), int(round(mask_y1))
                ...
                mask = torch.zeros((self.mask_size, self.mask_size)).long()
...
def get_loader(vocab, imgs_path, h5_path, batch_size, masks_json, mask_size=16, image_size = (256, 256), mode = 'train'):
    """Build and return data loader."""

    # dataset = CelebALoader(data_path, attr_list, mode, img_size)
    dataset = VGSceneLoader(vocab, imgs_path, h5_path, masks_json, mask_size=mask_size, image_size = image_size)
    shuffle = False
    if mode == 'train':
        shuffle = True
