...
def get_gan_losses(gan_type):
    ...
    elif gan_type == 'multi_lsgan':
        return multi_lsgan_g_loss, multi_lsgan_d_loss
    else:
...
def multi_lsgan_g_loss(scores_fake):
    loss = 0
    for score in scores_fake:
        loss += lsgan_g_loss(score[-1])
    
    return loss

def multi_lsgan_d_loss(scores_real, scores_fake):
    loss = 0
    for real_score, fake_score in zip(scores_real, scores_fake):
        loss += lsgan_d_loss(real_score[-1], fake_score[-1])
